...
def parseFolder(chamber: str, year: str):
    for (dirpath, dirnames, filenames) in walk(f"./raw/{chamber}/{year}"):
        for filename in filenames:
            rawFiles.append({"filename": filename, "chamber": chamber, "year": year})
        break


parseFolder("Senate", "2017")
parseFolder("House", "2017")
parseFolder("Senate", "2018")
parseFolder("House", "2018")
parseFolder("Senate", "2019")
parseFolder("House", "2019")


def cleanStr(str, removeNumbers=False):
...
def getOwner(line, lastLine=""):
    ...
    if "X DEPENDENT" in line:
        return "dependent"
    if "XFILER" in lastLine:
        return "filer"
    if "XSPOUSE" in lastLine:
        return "spouse"
    if "XDEPENDENT" in lastLine:
        return "dependent"
...
def parseFile(file):
    ...
    with open("./raw/" + chamber + "/" + year + "/" + fileName, newline="") as csvfile:
        ...
        for row in spamreader:
            ...
            if "Signature of Filer" in line:
                myDict["first_name"] = cleanStr(
                    lastRow.replace("The Honorable", "")
                    .replace("Mr.", "")
                    .replace("Ms.", "")
                    .replace("Mrs.", "")
                ).split(" ")[0]
            if "INCOMEPART 1A" in currentPart:
                if "INFORMATION RELATES TO" in lastRow:
                    myDict["occupational_income"].append(
                        {"held_by": getOwner(line, lastRow)}
                    )
            ...
            if "STOCKPART 2" in currentPart:
                ...
                if "STOCK HELD OR" in lastRow:
                    myDict["stocks"][-1]["held_by"] = getOwner(line, lastRow)
            ...
            if "INTERESTS IN REAL PROPERTYPART 7A" in currentPart:
                if "HELD OR ACQUIRED BY" in lastRow:
                    myDict["property"].append({"held_by": getOwner(line, lastRow)})
                if (
            ...
            if "MUTUAL FUNDSPART 4" in currentPart:
                if "MUTUAL FUND NAME" in lastRow:
                    currentMF = {"name": cleanStr(line)}
                if "SHARES OF MUTUAL FUND" in lastRow and "ACQUIRED BY " in line:
                    currentMF["held_by"] = getOwner(line, lastRow)
            ...
            if "GIFTSPART 8" in currentPart:
                ...
                if "RECIPIENT" in lastRow:
                    myDict["gifts"][-1]["recipient"] = getOwner(line, lastRow)
                if "DESCRIPTION OF GIFT" in line:
                    myDict["gifts"][-1]["description"] = cleanStr(
                        line.replace("DESCRIPTION OF GIFT", ""), removeNumbers=True
                    )
            ...
            if "BOARDS AND EXECUTIVE POSITIONSPART 12" in currentPart:
                ...
                if "POSITION HELD BY" in lastRow:
                    myDict["boards"][-1]["held_by"] = getOwner(line, lastRow)
            ...
            #         if len(section) and section[-1]["business_parties"]:
            ...
            if "BOND COUNSEL SERVICES PROVIDED BY A LEGISLATORPART 20" in currentPart:
                ...
                if "ISSUER NAME " in line:
                    section.append(
                        {
                            "issuer": cleanStr(
                                line.replace("ISSUER NAME", "").replace("1,", "")
                            )
                        }
                    )
                ...
                if "ISSUANCE AMOUNT " in line:
                    section[-1]["issuance_amount"] = cleanStr(
                        line.replace('3,"ISSUANCE AMOUNT', "").replace(
                            "ISSUANCE AMOUNT ", ""
                        )
                    )
            ...
            if "PART 11A" in currentPart:
                ...
                if "BUSINESS ASSOCIATION NAME AND ADDRESS" in lastLastRow:
                    section.append({"name": cleanStr(line)})
                ...
                if "BUSINESS TYPE " in lastRow and not section[-1]["type"]:
                    ...
                    # if not section[-1]["type"]:
                    #     print("line- ", line)
                    #     print("lastRow- ", lastRow)
                    #     print("lastLastRow- ", lastLastRow)
                    #     print("-")
                ...
                if (
                    ...
                    section[-1]["held_by"] = getOwner(line, lastRow)
